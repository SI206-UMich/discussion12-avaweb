...
def create_employee_table(cur, conn):
    cur.execute("CREATE TABLE IF NOT EXIST Employee (employee_id INT, first_name TEXT, hire_date TEXT, job_id INT, salary INT")
    conn.commit()
...
def job_and_hire_date(cur, conn):
    cur.execute('SELECT Employees.hire _date, Jobs.job_title FROM Employees JOIN Jobs ON Employees.job_id, = Jobs.job_id')
    job_hire_date = cur.fetchall()
    conn.commit()

    sorted_job_hire_date = sorted(job_and_hire_date, key = lambda x: x[0])
    return sorted_job_hire_date


# TASK 3: IDENTIFY PROBLEMATIC SALARY DATA
# Apply JOIN clause to match individual employees
def problematic_salary(cur, conn):
    cur.execute('SELECT Employees.first_name, Employees.last_name FROM Employees JOIN Jobs ON Employees.job_id = Jobs.job_id')
    conn.commit()
